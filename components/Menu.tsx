...
export const Menu = (props: MenuProps) => {
    ...
    const duplicateUser = () => {
        if (props?.users != undefined) {
            ...
            for (var i = 0; i < newArr.length; i++) {
                if (newArr[i].checked == true) { 
                    list.push({login: newArr[i].login + ' duplicate', id: Math.floor(Math.random() * 100000) + 1, avatar_url: newArr[i].avatar_url, checked: true});
                }
            }
            if (list.length > 0) {
  ...
  return (
    <>
    <TouchableOpacity onPress={() => {props?.setEdit(!props?.edit)}} style={styles.smBtn}>
        <Text>{props?.edit == true ? 'Cancel' : 'Edit'}</Text>
      </TouchableOpacity>
      {
        props?.edit == true &&
        <View style={styles.menu}>
          <TouchableOpacity style={(props?.selectAll == true || (props?.users && countSelected() == props?.users?.length && props?.users?.length > 0)) ? styles.checked : styles.uncheck} onPress={() => {
            setAllCheck();
          }}> 
          {
          (props?.selectAll == true ||  (props?.users != undefined && countSelected() == props?.users?.length && props?.users?.length > 0)) &&
